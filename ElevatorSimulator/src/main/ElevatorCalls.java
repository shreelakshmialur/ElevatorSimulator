...
public class ElevatorCalls implements Runnable, SchedulingAlgorithm {
        ...
        try {
            ...
            while (true) {
                if ("internal".equalsIgnoreCase(commandType)) {
                    synchronized (interiorButtonDummmy) {
                        active_buttons.add(Gui.internalButtonClicked);
                    ...
                    synchronized (ElevatorCalls.class) {
                        if(goToFloorEnteredByGui != -1) {
                            ...
                            chosen1.addQueue(goToFloorEnteredByGui);
                            active_buttons1.add(Gui.externalButtonClicked);
                            setExternal(goToFloorEnteredByGui,true);// System.out.println("This is callin from here External calling correctly");
    ...
    public static void setExternal(int floor,boolean value) {
    	if(value==false) {
    	    ListIterator<JButton> k = active_buttons1.listIterator();
